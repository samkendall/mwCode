...
export const PullRequestSummary = Schema.Struct({
  ...
  /**
   * The host's rolled-up review verdict. Rides along on the same single read the summary already
   * makes, so a linked thread can show "Approved"/"Changes requested" without the full detail.
   */
  reviewDecision: Schema.optional(PullRequestReviewDecision),
  headBranch: TrimmedNonEmptyString,
  baseBranch: TrimmedNonEmptyString,
