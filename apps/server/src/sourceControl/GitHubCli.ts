    ...
    getPullRequest: (input) =>
      execute({
        ...
        args: [
          ...
          input.reference,
          "--json",
          // `reviewDecision` rides along on this one read: a linked thread's row shows the
          // review verdict without paying for the full pull request detail.
          "number,title,url,baseRefName,headRefName,state,isDraft,reviewDecision,mergedAt,closedAt,updatedAt,isCrossRepository,headRepository,headRepositoryOwner",
        ],
      }).pipe(
        Effect.map((result) => result.stdout.trim()),
        Effect.flatMap((raw) =>
