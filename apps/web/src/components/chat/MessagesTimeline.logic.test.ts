...
describe("streaming row projection", () => {
  ...
  it("owns checkpoint lookups across streaming and equal source snapshots", () => {
    ...
    const input = {
      ...
      turnFoldOverrides: new Set([initial.historyTurnId]),
    ...
    const nextInput = {
      ...
      turnFoldOverrides: new Set(input.turnFoldOverrides),
  ...
  it("keeps full parity through completion, trailing failure, metadata, expansion, and paging", () => {
    ...
    check({ turnFoldOverrides: new Set([initial.historyTurnId, initial.turnId]) });
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps subagent spawn rows outside turn folds even after they settle", () => {
    ...
    const derive = (
      ...
      deriveMessagesTimelineRows({
        ...
        supportsConversationRollback: false,
        liveAgentTaskIds,
        ...(turnFoldOverrides ? { turnFoldOverrides } : {}),
      }).map((row) => row.id);
  ...
  it("keeps a settled turn's entries visible when auto-collapse is never", () => {
    ...
    expect(rows.map((row) => row.id)).toEqual([
      ...
      "turn-fold:turn-1",
      "assistant-first-entry",
      // A lone tool-like entry now renders as a plain work row rather than a
      // collapsed tool-group toggle; what matters here is that it stays visible.
      "work-entry-1",
      "assistant-final-entry",
    ]);
  ...
  it.each(["assistant", "user", "error", "turn"] as const)(
    ...
    (boundary) => {
      ...
      const rows = deriveMessagesTimelineRows({
        ...
        turnFoldOverrides: new Set([TurnId.make("turn-1"), TurnId.make("turn-2")]),
        isWorking: false,
        activeTurnStartedAt: null,
        turnDiffSummaries: [],
        supportsConversationRollback: false,
  ...
  it("folds mixed activity under worked-for and restores ordered details when expanded", () => {
    ...
    const expanded = deriveMessagesTimelineRows({
      ...input,
      turnFoldOverrides: new Set([TurnId.make("turn-1")]),
    });
  ...
  it("renders one tool call directly after collapsing its lifecycle updates", () => {
    ...
    const rows = deriveMessagesTimelineRows({
      ...
      turnFoldOverrides: new Set([turnId]),
      isWorking: false,
      activeTurnStartedAt: null,
      turnDiffSummaries: [],
      supportsConversationRollback: false,
  ...
  it("keeps user input in its own row through tool grouping and turn folding", () => {
    ...
    const expanded = deriveMessagesTimelineRows({ ...input, turnFoldOverrides: new Set([turnId]) });
