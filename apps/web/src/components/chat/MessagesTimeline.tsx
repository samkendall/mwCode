...
/** Turns folded the other way from the setting's default, tagged with that setting. */
interface TurnFoldOverrides {
  readonly setting: TranscriptAutoCollapse;
  readonly ids: ReadonlySet<TurnId>;
}
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  // makes the default, tagged with the setting they were recorded under. A
  // remembered set only restores under the setting it was recorded with.
  const rememberedTurnFoldOverrides = (): TurnFoldOverrides => ({
    setting: transcriptAutoCollapse,
    ids:
      rememberedPosition?.disclosures?.turnFoldSetting === transcriptAutoCollapse
        ? rememberedPosition.disclosures.turns
        : new Set(),
  });
  const [turnFoldOverrides, setTurnFoldOverrides] = useState<TurnFoldOverrides>(
    rememberedTurnFoldOverrides,
  );
  ...
  let paintedTurnFoldOverrides = turnFoldOverrides;
  ...
  if (listIdentityRef.current !== listIdentityKey) {
    ...
    paintedTurnFoldOverrides = rememberedTurnFoldOverrides();
    ...
    paintedExpandedReasoningMessageIds =
      rememberedPosition?.disclosures?.reasoningMessages ?? new Set();
    setTurnFoldOverrides(paintedTurnFoldOverrides);
  ...
  if (paintedTurnFoldOverrides.setting !== transcriptAutoCollapse) {
    paintedTurnFoldOverrides = { setting: transcriptAutoCollapse, ids: new Set() };
    setTurnFoldOverrides(paintedTurnFoldOverrides);
  }
  ...
  // Following a citation must leave its turn unfolded. An override inverts the
  // setting's default, so that means listing the turn while turns fold by
  // default and dropping it while they start expanded.
  const expandCitedTurn = useCallback((turnId: TurnId) => {
    setTurnFoldOverrides((existing) => {
      const needsOverride = !turnFoldsStartExpanded(existing.setting);
      if (existing.ids.has(turnId) === needsOverride) {
        return existing;
      }
    ...
    const projection = deriveMessagesTimelineRowsWithState(
      {
        ...
        latestTurn,
        runningTurnId,
        turnFoldOverrides: paintedTurnFoldOverrides.ids,
        transcriptAutoCollapse,
    ...
    if (position && state && isAtEnd !== undefined) {
      ...
      if (row && element) {
        rememberTimelinePosition(listIdentityKey, {
          ...
          atEnd: isAtEnd,
          disclosures: {
            turns: paintedTurnFoldOverrides.ids,
            turnFoldSetting: paintedTurnFoldOverrides.setting,
