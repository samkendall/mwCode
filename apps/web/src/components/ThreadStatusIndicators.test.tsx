...
describe("linked pull request snapshots", () => {
  ...
  it("uses the snapshot state and branches with the linked identity", () => {
    ...
    expect(result).toEqual({
      ...
      // A snapshot with no review or check rollup still answers, so the badge
      // reads "nothing to report" rather than falling back to a stale poll.
      signals: {
        isDraft: false,
        reviewDecision: null,
        checksState: null,
  ...
  it("carries the snapshot's review verdict to the badge without another read", () => {
    const result = linkedPullRequestSnapshotStatus({
      ...link,
      snapshot: {
        state: "open",
