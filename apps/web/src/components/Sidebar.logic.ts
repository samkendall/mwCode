 ...
 * is the arranged order itself (sortActiveThreadsByOrderKey): rows never
 * reshuffle as activity changes. Every other mode buckets threads into a coarse
 * tier (attention needed, work-type cluster, has-PR) and leaves the arranged
 * order intact inside each tier, so a re-sort can only move a row between tiers
 * — never jitter within one. Keys read straight off the thread shell, so no
 * mode adds a subscription. When grouping is on, the caller sorts before
 * grouping so this order becomes the within-group order.
 */
export function sortActiveThreadsForSidebar<T extends ActiveSortInput>(
  ...
  if (sortBy === "default") return arranged;
  ...
  // toSorted is stable, so threads sharing a tier keep the arranged order.
  return arranged.toSorted((left, right) => rankOf(left) - rankOf(right));
}
