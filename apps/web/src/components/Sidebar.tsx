...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  // The badge shade reflects the review/merge signals the linked PR already carries — merged,
  // ready-to-merge, checks failing, conflicts, auto-merge — read off its persisted snapshot, or
  // off the legacy summary poll for a server that predates link snapshots. A PR with neither
  // falls back to the coarse open/closed/merged state rather than inventing a verdict.
  ...
  // The shared badge control takes its color and label from the status it is handed, so the
  // review-aware shade rides in on that rather than forking the control.
  const prBadgeStatus =
    prStatus === null
      ? null
      : {
  ...
  if (variant === "slim") {
    // The sortable root props reach slim rows too: at compact density every
    // live row renders slim, and dragging must keep working there.
    return (
      <li
        data-thread-item
        {...sortableRootProps}
    ...
    <>
      {isRemote ? (
        <span className="inline-flex shrink-0 items-center text-sidebar-muted-foreground/70">
          <EnvironmentMachineIcon aria-hidden kind={props.environmentMachine} className="size-3.5" />
        </span>
      ) : null}
      {modelLabel ? (
        ...
        >
          <div className="relative z-10 h-[4.875rem] px-[var(--sidebar-row-content-inset)] py-[var(--sidebar-content-inset)]">
            <div className="flex h-5 min-w-0 items-center gap-1.5">
              ...
              {sortable?.isDragging ? dragDestination : topLineStatusSlot}
            </div>
            ...
            <div className="mt-0.5 flex min-w-0 items-center gap-1.5 text-secondary-label text-xs">
              {/* The worktree glyph is independent of the branch text: a
                  worktree is worth flagging even on a default branch. The
                  branch shows only when it's a real non-default one — a bare
...
export default function Sidebar() {
  ...
  const sidebarListItems = useMemo((): readonly SidebarListItem[] => {
    ...
    // Grouping renders the active run as headed blocks instead of loose rows,
    // and a heading is not a drop target. Leave those rows out of the measured
    // order so the drop planner never offers a slot the list cannot show.
    const activeRows =
      activeThreadGroups === null ? rowsOf(orderedActiveThreads, "active") : [];
                ...
                <SortableContext items={sortableIds} strategy={sidebarSortingStrategy}>
                  ...
                  >
                    {(() => {
                      ...
                      const renderGroupHeaderContent = (
                        ...
                        return (
                          <li
                            ...
                            className={cn(
                              "list-none",
                              bag?.isDragging && "relative z-20 opacity-80",
                            )}
                          >
                            <div
                            ...
                            >
                              {header.kind === "project" ? (
                                ...
                                <span
                                  ...
                                  style={
                                    header.color ? { backgroundColor: header.color } : undefined
                                  }
                                />
                              )}
                              <span
                      ...
                      // Grouping replaces the flat active run with headed blocks.
                      // Those rows leave the list-wide sortable order (see
                      // sidebarListItems), because a heading is not a drop target
                      // and a grouped row's position is the group's sort, not a
                      // slot the user can drag it into. Pinned, snoozed and
                      // settled rows keep dragging either way.
                      const groupedActiveBlock =
                        activeThreadGroups === null ? null : sidebarGroupBy === "project" ? (
                          // Grouped by project: headers drag to reorder (persisted
                          // to sidebarProjectManualOrder) and each carries a
                          // per-project sort button. The project headers are the
                          // sortable items of their own nested context; the null
                          // "Other" bucket is not draggable.
                          <li key="active-project-groups" className="list-none">
                            ...
                            >
                              <SortableContext
                                items={activeThreadGroups
                                  .filter(
                                    (group) =>
                                      group.header.kind === "project" && group.header.project,
                                  )
                                  .map((group) => group.key)}
                              ...
                              >
                                <ul role="list" className="flex flex-col gap-px">
                                  {activeThreadGroups.map((group) => {
                                    const draggable =
                                      group.header.kind === "project" &&
                                      group.header.project !== null;
                                    return (
                          ...
                          <Fragment key="active-taxonomy-groups">
                            {activeThreadGroups.map((group) => (
                              <Fragment key={`active-group:${group.key}`}>
                                {renderGroupHeaderContent(group, null)}
                                {/* Taxonomy groups keep per-row identity (the
                                    header names the type, not the project). */}
                                {group.threads.map((thread) =>
                                  renderThreadRowInner(
                                    thread,
                                    "active",
                      ...
                      for (const item of sidebarListItems) {
                        ...
                        switch (item.marker) {
                          ...
                          case "active-placeholder":
                            ...
                            if (groupedActiveBlock !== null) items.push(groupedActiveBlock);
                            break;
                          case "snoozed-header":
                            items.push(
