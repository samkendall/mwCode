...
stdout/stderr at `~/.mwcode/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.mwcode/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.mwcode/userdata/...` by default, or
...
worktree uses `~/.mwcode/dev/logs/server.trace.ndjson`.
...
```bash
TRACE_FILE="${T3CODE_HOME:-$HOME/.mwcode}/userdata/logs/server.trace.ndjson"
```
...
```bash
TRACE_FILE="$HOME/.mwcode/dev/logs/server.trace.ndjson"
```
