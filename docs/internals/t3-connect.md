...
Local desktop development uses `mwcode-dev://app`, while packaged builds use `mwcode://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `mwcode-dev://app`; the production Clerk instance should only need
`mwcode://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
