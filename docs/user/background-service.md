...
| `service-disabled` or `service-stopped` | Read the log and `systemctl --user status mwcode.service`, then use the repair command printed by T3 Code.                     |
...
`ProgramArguments` in
`~/Library/LaunchAgents/net.samkendall.mwcode.service.plist`.
